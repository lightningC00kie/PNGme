...
impl TryFrom<&[u8]> for Chunk {
    ...
    fn try_from(value: &[u8]) -> Result<Self, &'static str> {
        let length: u32 = value[0..4] as u32;
        let chunk_type = ChunkType::try_from(value[4..8]).unwrap(); 
        let data_end_idx = value.len() - 4; 
        let chunk_data = value[8..data_end_idx];
        let crc = value[data_end_idx..] as u32;
